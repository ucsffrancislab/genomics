...
--all-score \
    ...
    --stat OR\
    --or \
    --out EUR > PRSice.R.out 2>&1

cat PRSice.R.out | tr '\r' '\n' | grep -vs '^Processing .*%$' | grep -vs 'Clumping Progress: .*%$'
```
...
How is the best-fit selected?

Are the notebooks above results the same as mine?
...
#	Seems to try to remove the file as soon as set?
#	Warning in file.remove(paste0(tmp, ".sbk")): cannot remove file
#	'tmp-data/file6781112f5d891.sbk', reason 'No such file or directory'

#	files do get created in ./tmp-data/ but they do not get removed?
...
#	Don't think that I need it anymore. Why not just delete it?
on.exit(file.remove(paste0(tmp, ".sbk")), add = TRUE)
```
...
Infinitesimal = 0.0248696 <- is this approximate? my value, seen here, was 0.0545, if I'm reading it correctly. I don't think that I am.

Grid Model = 0.001746926 <- not run here

Auto Model = 0.1751478 <- not run here
