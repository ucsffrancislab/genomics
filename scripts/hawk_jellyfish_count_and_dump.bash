...
if [ -f $f1 ] && [ ! -w $f1 ] ; then
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		then
			...
			hawk_jellyfish merge -o ${f} ${OUTBASE}/tmp*
		fi
		rm -rf ${OUTBASE}
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		if [ -f $f2 ] && [ ! -w $f2 ] ; then
			...
			echo "Creating $f2"
			hawk_jellyfish histo --full --output ${f2} --threads ${threads} ${OUTBASE}.jellyfish
			chmod a-w $f2
		fi
	...
	if [ -f $f2 ] && [ ! -w $f2 ] ; then
		...
		if [ -f $f3 ] && [ ! -w $f3 ] ; then
			...
			echo "Creating $f3"
			hawk_jellyfish dump --column --lower-count 2 ${OUTBASE}.jellyfish > ${f3}
			chmod a-w $f3
		fi
