...
#[gwendt@c4-log1 /francislab/data1/raw/20250822-Illumina-PhIP]$ head -1 L5_full_covariates_Vir3_phip-seq_GBM_ALL_p15_and_p16_8-22-25hmh.csv 
...
#head /francislab/data1/raw/20250822-Illumina-PhIP/manifest.csvSequencer S#,Avera Sample_ID,Avera RunName,Index primer,Index 'READ',UCSF sample name (PRN BlindID/PLCO liid),UCSF sample name for sequencing (PRN BlindID/PLCO liid),Sample type,Study,Analysis group (PLCO and PRN),PLCO barcode [GBM]/PRN tube no [ALL] /IPS kitno [Plate 4 IPS GBM repeats],sex,age,best_draw_label (PLCO),match_race7 (PLCO),self-identified race/ethnicity (PRN),M_BLINDID (PRN),BIRTH_YEAR (PRN),Matching Race (IPS case),IDH mut (IPS case),dex_draw (IPS case),dex_prior_month (IPS case),Timepoint (IPS cases),192 sequencing Lane,Plate,well,column order,,
...
echo -n "Plate"
for s in ${samples} ; do
	v=$( awk -F, -v sample=${s} '( $1 == sample ){print $25}' ${manifest} )
	echo -n ",${v}"
done
echo
...
echo -n "Well X"
for s in ${samples} ; do
	v=$( awk -F"," -v sample=${s} '( $1 == sample ){x=substr($26,1,1); print x}' ${manifest} | tr 'A-H' '1-8' )
	echo -n ",${v}"
done
echo
...
echo -n "Well Y"
for s in ${samples} ; do
	v=$( awk -F"," -v sample=${s} '( $1 == sample ){x=substr($26,2,2); print int(x)}' ${manifest} )
	echo -n ",${v}"
done
echo
