...
for filename in args.files:  
	...
	if os.path.isfile(filename) and os.path.getsize(filename) > 0:
		...
		#	4.count.Zscores.2-3-hits.found_public_epitopes.BEFORE_scoring.seropositive.csv
		basename = re.sub(".count.Zscores.", " ", basename, flags=re.IGNORECASE) 
		basename = re.sub("-hits.found_public_epitopes.", " ", basename, flags=re.IGNORECASE) 
		basename = re.sub("_scoring.seropositive.csv", "", basename, flags=re.IGNORECASE) 
		basename = re.sub("-hits.virus_scores.csv", "", basename, flags=re.IGNORECASE) 
