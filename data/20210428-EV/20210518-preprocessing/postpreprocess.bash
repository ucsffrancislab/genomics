...
#	fi
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		awk '(NR==FNR){t2g[$1]=$2}(NR!=FNR){print t2g[$1]}' ${transcript_gene} \
			<( zcat ${bam}.aligned_sequences.txt.gz ) | sort --parallel=8 | uniq -c | sort -rn > ${f}
#			${bam}.transcript_ids | sort --parallel=8 | uniq -c | sort -rn > ${f}
		chmod -w $f
	fi
done

f=post/gene_counts.csv.gz
if [ -f $f ] && [ ! -w $f ] ; then
	echo "Write-protected $f exists. Skipping."
else
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/gene_counts.csv \
		output/*gene_counts
	gzip ${f%.gz}
	chmod -w $f
fi
...
#	awk '(NR==FNR){t2g[$1]=$2}(NR!=FNR){print t2g[$1]}' ${transcript_gene} \
#		<( zcat output/*.toTranscriptome.out.bam.aligned_sequences.txt.gz ) \
...
#	zcat output/*.STAR.mirna.Aligned.sortedByCoord.out.bam.aligned_sequences.txt.gz \
...
f=post/mirna_counts.csv.gz
if [ -f $f ] && [ ! -w $f ] ; then
	echo "Write-protected $f exists. Skipping."
else
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/mirna_counts.csv \
		output/*STAR.mirna.Aligned.sortedByCoord.out.bam.aligned_sequence_counts.txt
	#	output/*STAR.mirna.Aligned.sortedByCoord.out.bam.mirna_counts
	gzip ${f%.gz}
	chmod -w $f
fi
...
f=post/rmsk_family_counts.csv.gz
if [ -f $f ] && [ ! -w $f ] ; then
	echo "Write-protected $f exists. Skipping."
else
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/rmsk_family_counts.csv \
		output/*rmsk_family_counts
	gzip ${f%.gz}
	chmod -w $f
fi

f=post/rmsk_class_counts.csv.gz
if [ -f $f ] && [ ! -w $f ] ; then
	echo "Write-protected $f exists. Skipping."
else
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/rmsk_class_counts.csv \
		output/*rmsk_class_counts
	gzip ${f%.gz}
	chmod -w $f
fi

f=post/rmsk_name_counts.csv.gz
if [ -f $f ] && [ ! -w $f ] ; then
	echo "Write-protected $f exists. Skipping."
else
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/rmsk_name_counts.csv \
		output/*rmsk_name_counts
	gzip ${f%.gz}
	chmod -w $f
fi
...
#	zcat output/*.bowtie2.mRNA_Prot.bam.aligned_sequences.txt.gz \
...
if [ -f $f ] && [ ! -w $f ] ; then
	...
	#	output/*STAR.mirna.Aligned.sortedByCoord.out.bam.aligned_sequence_counts.txt
	##	output/*STAR.mirna.Aligned.sortedByCoord.out.bam.mirna_counts
	#gzip post/mirna_counts.csv
	#python3 ~/.local/bin/merge_uniq-c.py --int --output post/mrna_counts.csv output/*mrna_counts
	python3 ~/.local/bin/merge_uniq-c.py --int \
		--output post/mrna_counts.csv \
		output/*.bowtie2.mRNA_Prot.bam.aligned_sequence_counts.txt
	gzip post/mrna_counts.csv
	chmod -w $f
fi
