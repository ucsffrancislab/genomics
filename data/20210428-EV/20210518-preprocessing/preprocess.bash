...
for fastq in /francislab/data1/working/20210428-EV/20210511-trimming/trimmed/SFHH00*.fastq.gz ; do
	...
	for trimmer in bbduk1 bbduk2 bbduk3 cutadapt1 cutadapt2 cutadapt3 ; do
		...
		#separate_id=${trim_id}
		in_base=${out_base}
		long_out_base=${in_base}.gt30		#.fastq.gz
		short_out_base=${in_base}.lte30	#.fastq.gz
		...
		if [ -f $f ] && [ ! -w $f ] ; then
			...
			separate_id=$( ${sbatch} ${depend} --parsable --job-name=${basename}.${t}sep \
				--time=180 --ntasks=2 --mem=15G \
				--output=${out_base}.${date}.txt \
						${PWD}/fastq_separate_by_length.bash ${in_base}.fastq.gz )
			echo ${separate_id}
