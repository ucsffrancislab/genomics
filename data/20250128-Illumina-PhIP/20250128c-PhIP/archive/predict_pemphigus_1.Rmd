...
```{python}
label = ['Endemic Control', 'Non Endemic Control', 'PF Patient']
ax = sns.heatmap(c_matrix, annot=True, fmt='d', cmap='Greens', xticklabels=label, yticklabels=label)
ax.set_title('Confusion Matrix for Pemphigous Zscores Dataset')
...
#get_gradients = K.function([input_tensor], K.gradients(output_tensor, input_tensor))
#
## Calculate integrated gradients for a single sample
#gradients = get_gradients([x_test[0]]) 
#
## Aggregate the gradients to get feature importances
#feature_importances = np.mean(np.abs(gradients[0]), axis=0) 
...
for i, importance in enumerate(feature_importance[:100]):
