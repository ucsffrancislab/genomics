...
ggplot(localdf, aes(y = -log10(pval), x = exp(beta), col = group, size=size, alpha=alpha)) +
	...
	geom_point() +
	geom_text(aes(label=as.character(label)), hjust=0.5, nudge_y = 0.1)+
	coord_cartesian(ylim = c(0, 4), xlim = c( 0.5, 1.5)) +
	labs(x="exp(beta)", y=expression(paste("-log"[10]*"(",italic("P"),")"))) +
	scale_color_brewer(palette="Set1") +
	theme_bw() +
	theme(panel.grid.minor = element_blank(),
		legend.title = element_text(size=14, color="black",lineheight = 1),
...
ids <- data.frame(data.table::fread('/francislab/data1/working/20250128-Illumina-PhIP/20250128c-PhIP/HHV8-ORF73.ids.txt', sep = ",", header=TRUE))

localdf = localdf[localdf$species==title,]
localdf = localdf[localdf$peptide %in% ids$id,]
