...
IN=/francislab/data1/raw/20200407_Schizophrenia
...
for r1 in ${IN}/??_R1.fastq.gz ; do
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		bbduk_id=$( qsub -N ${jobbase}.bbduk -l nodes=1:ppn=4 -l vmem=32gb \
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		length_id=$( qsub ${depend} -N ${jobbase}.length -l nodes=1:ppn=4 -l vmem=8gb \
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		qsub ${depend} -N ${jobbase}.unpair -l nodes=1:ppn=4 -l vmem=8gb \
