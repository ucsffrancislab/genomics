...
if(nrow(metal_results) > 0) {
  ...
  bonf_threshold <- -log10(0.05 / nrow(metal_results))
  
  # Identify models with points above threshold
  models_above_threshold <- metal_plot %>%
    filter(neglog10p > bonf_threshold) %>%
    pull(MarkerName) %>%
    unique()
  
  # For custom models (few), show all labels; otherwise only significant
  if(length(unique(metal_plot$MarkerName)) <= 20) {
    # Few models - show all labels
    model_positions$label <- model_positions$MarkerName
  } else {
    # Many models - only label those above threshold
    model_positions$label <- ifelse(model_positions$MarkerName %in% models_above_threshold,
  ...
  ggplot(metal_plot, aes(x = model_num, y = neglog10p, color = clean_subset)) +
    ...
    labs(title = "Manhattan Plot: Meta-Analysis Results Across All PRS Models",
         subtitle = "Red line = Bonferroni threshold; Models sorted alphabetically; Colors = case subsets",
         x = "PRS Model", y = "-log10(P-value)",
         color = "Case Subset") +
...
## Kaplan-Meier Curves: All Datasets × All Models (Style 1)

**This section generates:** 4 datasets × 10 models = **40 Kaplan-Meier plots**

Each plot shows survival stratified by PRS quartiles (Q1-Q4).

```{r km_curves_example, fig.height=10, eval=TRUE, results='asis'}
# Get all custom models
all_custom_models <- grep("glioma|gbm|idh", names(zscores_list[[1]]), 
                         value = TRUE, ignore.case = TRUE)

cat("\nGenerating", length(datasets), "datasets ×", length(all_custom_models), 
    "models =", length(datasets) * length(all_custom_models), "Kaplan-Meier plots\n\n")
    ...
    for(model in all_custom_models) {
      if(model %in% names(zscores_list[[ds]])) {
        ...
        tryCatch({
          ...
          km_data <- merge(surv_df[, c("sample", "survdays", "vstatus")],
                          score_df[, c("sample", model)],
                          by = "sample")
          
          # Create risk groups (quartiles)
          km_data$risk_group <- cut(km_data[[model]],
                                    breaks = quantile(km_data[[model]], 
          ...
          # Remove missing data
          km_data <- km_data[complete.cases(km_data[, c("survdays", "vstatus", model)]), ]
          ...
          if(nrow(km_data) > 10) {
            ...
            p <- ggsurvplot(fit,
                       ...
                       conf.int = FALSE,
                       risk.table = TRUE,
                       risk.table.height = 0.25,  # Reduced from 0.25
                       title = paste(toupper(ds), "-", model),
                       xlab = "Time (days)",
            ...
            cat("Insufficient data (n=", nrow(km_data), ")\n\n")
          }
...
cat("\n**Total KM plots generated:**", plot_count, "\n\n")
```

## Kaplan-Meier Template for Custom Models (Alternative Style)

**This section generates:** 4 datasets × 10 models = **40 Kaplan-Meier plots**

Alternative visualization using a function-based approach with different styling options.

```{r km_curves_template, fig.height=10, eval=TRUE, results='asis'}
# Define KM plotting function
create_km_plot <- function(dataset_name, prs_model, survival_df, scores_df) {
  ...
  # Remove missing data
  plot_data <- plot_data[complete.cases(plot_data[, c("survdays", "vstatus", prs_model)]), ]
  ...
  # Plot with different styling than Example section
  ggsurvplot(fit,
             data = plot_data,
             pval = TRUE,
             pval.coord = c(0.1, 0.1),
             conf.int = TRUE,  # Different: Shows confidence intervals
             risk.table = TRUE,
             risk.table.height = 0.25,  # Reduced from 0.3
             title = paste(dataset_name, "-", prs_model),  # Different: Keep full model name
...
cat("\n**Total KM plots generated (Template style):**", plot_count, "\n\n")
```

## Multi-Dataset Kaplan-Meier Comparison - All Models

**Risk Stratification:** Binary split at median (Low vs High), calculated within each dataset.

**This section generates:** 10 multi-dataset comparison plots (one per model).

```{r km_multi_dataset, fig.height=10, eval=TRUE, results='asis'}
# Function to create multi-dataset KM
create_multi_km <- function(prs_model) {
  all_data_list <- list()
  ...
  for(ds in datasets) {
    if(!is.null(survival_data_list[[ds]]) && !is.null(zscores_list[[ds]])) {
      if(prs_model %in% names(zscores_list[[ds]])) {
        ...
        # Binary risk groups (median split within dataset)
  ...
  # Fit with dataset stratification
...
cat("\nCreating multi-dataset comparison for", length(all_custom_models), "models\n\n")

# Generate for ALL 10 models
plot_count <- 0
for(model in all_custom_models) {
  cat("\n## ", model, "\n\n")
  
  tryCatch({
    km_result <- create_multi_km(model)
...
cat("\n**Total multi-dataset plots generated:**", plot_count, "\n\n")
```
