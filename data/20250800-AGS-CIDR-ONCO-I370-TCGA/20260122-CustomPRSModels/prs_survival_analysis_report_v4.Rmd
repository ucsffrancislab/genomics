...
if(nrow(metal_results) > 0) {
  ...
  # Create x-axis by model (not subset)
  metal_plot <- metal_results %>%
    mutate(model_num = as.numeric(factor(MarkerName))) %>%
  ...
  ggplot(metal_plot, aes(x = model_num, y = neglog10p, color = clean_subset)) +
    ...
    labs(title = "Manhattan Plot: Meta-Analysis Results Across All PRS Models",
         subtitle = "Red line = Bonferroni threshold; X-axis = PRS models; Each color = case subset",
         x = "PRS Model (ordered)", y = "-log10(P-value)",
         color = "Case Subset") +
    theme(axis.text.x = element_blank(),  # Too many models to label individually
        ...
        tryCatch({
          ...
          if(nrow(km_data) > 10) {
            ...
            p <- ggsurvplot(fit,
                       ...
                       conf.int = FALSE,
                       risk.table = TRUE,
                       risk.table.height = 0.15,  # Reduced from 0.25
...
create_km_plot <- function(dataset_name, prs_model, survival_df, scores_df) {
  ...
  ggsurvplot(fit,
             ...
             risk.table = TRUE,
             risk.table.height = 0.15,  # Reduced from 0.3
