...
if(nrow(metal_results) > 0) {
  ...
  # Clean subset names for display
  subset_centers$clean_label <- gsub("_meta_cases|_meta", "", subset_centers$subset)
  metal_plot$clean_subset <- gsub("_meta_cases|_meta", "", metal_plot$subset)
  
  ggplot(metal_plot, aes(x = x_plot, y = neglog10p, color = clean_subset)) +
    geom_point(alpha = 0.6, size = 1.5) +
    geom_hline(yintercept = -log10(0.05 / nrow(metal_results)), 
               linetype = "dashed", color = "red", size = 1) +
    scale_x_continuous(breaks = subset_centers$center, 
                       labels = subset_centers$clean_label) +
    ...
    labs(title = "Manhattan Plot: Meta-Analysis Results Across All Subsets",
         subtitle = "Red line = Bonferroni threshold; Each color = different case subset",
         x = "Case Subset", y = "-log10(P-value)",
         color = "Subset") +
