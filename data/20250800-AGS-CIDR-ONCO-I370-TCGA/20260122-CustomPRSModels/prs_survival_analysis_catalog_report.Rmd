...
non_zero_models <- names(model_variances)[model_variances > 0 & !is.na(model_variances)]
model_variances_filtered <- model_variances[non_zero_models]

# Select top 5 highest variance and bottom 5 lowest variance (from non-zero)
top_var_models <- names(sort(model_variances_filtered, decreasing = TRUE))[1:min(5, length(model_variances_filtered))]
bottom_var_models <- names(sort(model_variances_filtered, decreasing = FALSE))[1:min(5, length(model_variances_filtered))]
sample_prs <- c(top_var_models, bottom_var_models)

cat("Showing models with highest and lowest variance (excluding zero variance)
")
cat("Highest variance:", paste(top_var_models, collapse = ", "), "
")
cat("Lowest variance:", paste(bottom_var_models, collapse = ", "), "
")
cat("Zero variance models excluded:", sum(model_variances == 0 | is.na(model_variances)), "
...
# Note: scores.info files are in the original pgs-calc directory, not merged
original_base_path <- "/francislab/data1/working/20250800-AGS-CIDR-ONCO-I370-TCGA/20260122-CustomPRSModels/pgs-calc-scores-new_models-claude"

coverage_data_list <- lapply(datasets, function(ds) {
  info_file <- file.path(original_base_path, ds, "scores.info")
  if(file.exists(info_file)) {
    ...
    tryCatch({
      ...
      cat("Could not read", info_file, ":", e$message, "\n")
      return(NULL)
    })
  } else {
    cat("File not found:", info_file, "\n")
  }
  return(NULL)
})
...
if(nrow(coverage_data) > 0) {
  ...
  # Distribution plot
  p <- ggplot(coverage_data, aes(x = coverage, fill = dataset)) +
...
- **X-axis labels:** With ~5,000 models, only top 50 most significant are labeled to maintain readability
- **Points at top (~300):** These represent p-values < 1e-300 (machine precision limit)
- **Many significant hits:** This dataset shows strong PGS associations with glioma survival
- **Colors:** Different case subsets (ALL, HGG, LrGG, IDH status, 1p19q status)
...
if(nrow(metal_results) > 0) {
  ...
  # Only label TOP 50 most significant models (not all Bonferroni significant)
  # With 5000+ models, 386 Bonferroni hits is too many to label
  ...
  cat("  Bonferroni significant:", sum(metal_plot$neglog10p > bonf_threshold, na.rm = TRUE) / n_subsets, "models\n")
  cat("  Top 50 most significant labeled\n\n")
  
  # Note: Many points at top = extremely small p-values (< 1e-300)
  n_extreme <- sum(metal_plot$neglog10p > 300, na.rm = TRUE)
  if(n_extreme > 0) {
    cat("Note:", n_extreme, "associations have p < 1e-300 (appear at top of plot)\n\n")
  }
  ...
  ggplot(metal_plot, aes(x = model_num, y = neglog10p, color = clean_subset)) +
    ...
    labs(title = "Manhattan Plot: Meta-Analysis Results Across All PGS Models",
         subtitle = paste("Models sorted alphabetically; Top 50 most significant labeled; Colors = case subsets"),
...
**Note on extreme values:** Some PGS models show extremely small p-values (< 1e-300) and correspondingly extreme hazard ratios. These represent very strong associations but HR values > 1e6 should be interpreted cautiously as they may indicate model overfitting or data quality issues.
...
if(nrow(metal_results) > 0) {
  ...
  top_meta <- metal_results %>%
    ...
    slice_head(n = 50) %>%
    mutate(
      # Flag extreme HR values
      HR_display = ifelse(Meta_HR > 1e6, sprintf(">1e6 (%.2e)", Meta_HR), 
                   ifelse(Meta_HR < 1e-6, sprintf("<1e-6 (%.2e)", Meta_HR),
                          sprintf("%.3f", Meta_HR)))
    ) %>%
    select(subset, MarkerName, Effect, Pvalue, Meta_HR, HR_display, HetPVal)
  
  datatable(top_meta,
            caption = "Top 50 PRS from Meta-Analysis (All Subsets)",
            options = list(pageLength = 25),
            rownames = FALSE,
            colnames = c("Subset", "PGS Model", "Effect", "P-value", "Meta HR", "HR (Display)", "Het P")) %>%
    formatRound(columns = c("Effect"), digits = 3) %>%
    formatSignif(columns = c("Pvalue", "Meta_HR", "HetPVal"), digits = 3) %>%
    formatStyle('Meta_HR',
                backgroundColor = styleInterval(c(1e6), c('white', 'lightyellow')))
  
  # Summary of extreme values
  n_extreme <- sum(top_meta$Meta_HR > 1e6 | top_meta$Meta_HR < 1e-6, na.rm = TRUE)
  if(n_extreme > 0) {
    cat("\n**Warning:**", n_extreme, "of top 50 hits have extreme HR values (>1e6 or <1e-6)\n")
    cat("These may indicate:\n")
    cat("- Very strong true associations\n")
    cat("- Model overfitting or separation issues\n")
    cat("- Data quality concerns\n")
    cat("Review individual model results carefully\n\n")
  }
} else {
...
```{r forest_plots_top, fig.height=18, fig.width=14}
if(nrow(metal_results) > 0 && nrow(cox_results) > 0) {
  ...
  if(length(top_models) > 0) {
    # Create forest plots for top models in same style as custom
    forest_plots <- lapply(top_models, function(model) {
      ...
      meta_result <- metal_results %>%
        filter(MarkerName == model, subset == "ALL_meta_cases") %>%
        select(Effect, StdErr, Pvalue, HetPVal) %>%
        mutate(
          dataset = "META",
          HR = exp(Effect),
          CI_lower = exp(Effect - 1.96 * StdErr),
      ...
      # Combine
      plot_data <- bind_rows(model_data, meta_result) %>%
        mutate(dataset = factor(dataset, levels = c("META", "tcga", "i370", "onco", "cidr")))
      
      # Get meta p-value and het p for subtitle
      meta_p <- meta_result$Pvalue[1]
      het_p <- if("HetPVal" %in% names(meta_result)) meta_result$HetPVal[1] else NA
      
      # Plot in same style as custom
      ggplot(plot_data, aes(x = HR, y = dataset, color = dataset)) +
        ...
        labs(title = gsub("_scoring_system|_", " ", model),
    ...
    # Display in 2-column grid like custom forest plots
    do.call(grid.arrange, c(forest_plots, ncol = 2))
  }
}
```
  ...
  for(ds in datasets) {
    if(!is.null(survival_data_list[[ds]]) && !is.null(zscores_list[[ds]])) {
      if(prs_model %in% names(zscores_list[[ds]])) {
        ...
        # Quartile risk groups (within dataset) - with error handling
        tryCatch({
          # Check if we have enough unique values for quartiles
          unique_vals <- length(unique(merged[[prs_model]][!is.na(merged[[prs_model]])]))
          
          if(unique_vals < 4) {
            # Not enough unique values for quartiles, use binary split
          ...
          # If quartile calculation fails, use binary split as fallback
          median_val <- median(merged[[prs_model]], na.rm = TRUE)
          merged$risk_group <<- ifelse(merged[[prs_model]] <= median_val, "Low", "High")
        })
