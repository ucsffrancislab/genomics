...
sample_prs <- sample(setdiff(all_models, c(comma_models, split_models)), 
                     min(20, length(setdiff(all_models, c(comma_models, split_models)))))
...
metal_results_list <- lapply(metal_files, function(f) {
  ...
  df <- fread(f, colClasses = list(character = "P-value"))  # Read P-value as character
  df$`P-value` <- as.numeric(df$`P-value`)  # Convert to numeric
  df$subset <- subset
  return(df)
})
...
# Remove any rows/columns with NA, NaN, or Inf
...
if(nrow(heat_data) < 2 || ncol(heat_data) < 2) {
  cat("Insufficient data for heatmap (need at least 2x2 matrix)\n")
} else {
  pheatmap(heat_data,
