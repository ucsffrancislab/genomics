...
for i in [5,17,22]:
    ...
    pgscalc = pd.read_csv(f'pgs-calc-scores/cidr/chr{i}.scores.txt')
    ...
    for model in models:
        # Load plink scores
        #plink = pd.read_csv(f'plink-scores/imputed-umich-cidr.{model}.prs.genome_raw.txt', 
        plink = pd.read_csv(f'plink-scores/imputed-umich-cidr/chr{i}.dose.{model}.sscore',
                            sep='\t')
