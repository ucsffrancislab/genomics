...
sed -e "s/_SUBSETNAME/_${subset}/g" ${PWD}/${FILE} > $TMPDIR/$( basename $FILE )

cat $TMPDIR/$( basename $FILE )

echo metal $TMPDIR/$( basename $FILE )
#metal $TMPDIR/$( basename $FILE ) > ${PWD}/metal_survival_${server}_${subset}_1.log
metal $TMPDIR/$( basename $FILE ) > ${PWD}/metal_survival_${subset}_1.log
