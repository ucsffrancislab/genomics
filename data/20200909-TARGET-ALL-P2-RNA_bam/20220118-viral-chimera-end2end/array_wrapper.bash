...
IN="/francislab/data1/working/20200909-TARGET-ALL-P2-RNA_bam/20220104-viral-chimera/out"
OUT="/francislab/data1/working/20200909-TARGET-ALL-P2-RNA_bam/20220118-viral-chimera-end2end/out"
...
if [ -f $f ] && [ ! -w $f ] ; then
	...
	~/.local/bin/paired_reads_select_scratch.bash \
		...
		--very-sensitive -1 ${r1} -2 ${r2} -o ${f%.R1.fastq.gz}.bam
...
if [ -d $f ] ; then
	...
	echo "Creating $f"
	~/.local/bin/bam_to_fastq_by_refseq.bash ${f}.bam
fi
...
for r1 in ${inbase}/*.R1.fastq.gz ; do
	...
	if [ -f $f ] && [ ! -w $f ] ; then
		...
		~/.local/bin/bowtie2.bash --sort --threads 8 \
			-x /francislab/data1/refs/sources/hgdownload.cse.ucsc.edu/goldenPath/hg38/bigZips/latest/hg38.chrXYM_no_alts \
			--very-sensitive -1 ${r1} -2 ${r1/.R1./.R2.} -o ${f}
...
mkdir -p /francislab/data1/working/20200909-TARGET-ALL-P2-RNA_bam/20220118-viral-chimera-end2end/logs
date=$( date "+%Y%m%d%H%M%S" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-532%8 --job-name="viralchimera" --output="/francislab/data1/working/20200909-TARGET-ALL-P2-RNA_bam/20220118-viral-chimera-end2end/logs/array.${date}-%A_%a.out" --time=4320 --nodes=1 --ntasks=8 --mem=60G --gres=scratch:250G /francislab/data1/working/20200909-TARGET-ALL-P2-RNA_bam/20220118-viral-chimera-end2end/array_wrapper.bash
