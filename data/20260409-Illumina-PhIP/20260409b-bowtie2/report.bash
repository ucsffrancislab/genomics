...
rawdir=/francislab/data1/raw/20260409-Illumina-PhIP/fastq
cutdir=/francislab/data1/working/20260409-Illumina-PhIP/20260409a-cutadapt/out
bowdir=/francislab/data1/working/20260409-Illumina-PhIP/20260409b-bowtie2/out

#manifest=/francislab/data1/raw/20260409-Illumina-PhIP/manifest.csv
covariates=/francislab/data1/raw/20260409-Illumina-PhIP/select_covariates.csv

#	head -1 /francislab/data1/raw/20260409-Illumina-PhIP/select_covariates.csv | tr ',' '\n' | awk '{print NR,$0}'
#	1 snumber
#	2 sample
#	3 subject
#	4 type
...
echo -n "Subject"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $3}' ${covariates} )
	v=${v/dup/}
	echo -n ",${v}"
done
echo
...
echo -n "Name"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $2}' ${covariates} )
	echo -n ",${v}"
done
echo
...
#echo -n "Index"
#for s in ${snumbers} ; do
#	name=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $3}' ${covariates} )
...
echo -n "Type"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $4}' ${covariates} )

#	subject=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $2}' ${covariates} )
...
echo -n "Study"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $5}' ${covariates} )
	echo -n ",${v}"
done
echo
...
echo -n "Group"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $8}' ${covariates} )
	echo -n ",${v}"
done
echo
...
echo -n "Age"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $7}' ${covariates} )
	echo -n ",${v}"
done
echo
...
echo -n "Sex"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $6}' ${covariates} )
	echo -n ",${v}"
done
echo
...
echo -n "Plate"
for s in ${snumbers} ; do
	v=$( awk -F, -v snumber=${s} '( $1 == snumber ){print $9}' ${covariates} )
#	v=99
	echo -n ",${v}"
done
echo
...
echo -n "Well X"
for s in ${snumbers} ; do
	v=$( awk -F"," -v snumber=${s} '( $1 == snumber ){x=substr($10,1,1); print x}' ${covariates} | tr 'A-H' '1-8' )
	echo -n ",${v}"
done
echo
...
echo -n "Well Y"
for s in ${snumbers} ; do
	v=$( awk -F"," -v snumber=${s} '( $1 == snumber ){x=substr($10,2,2); print int(x)}' ${covariates} )
	echo -n ",${v}"
done
echo
...
for s in ${snumbers} ; do
	c=$( cat ${bowdir}/${s}.VIR3_clean.id_upper_oligo.uniq.1-80.bam.aligned_sequence_counts.q40.txt 2> /dev/null | wc -l )
	echo -n ",${c}"
done
echo
...
for s in ${snumbers} ; do
	c=$( cat ${bowdir}/${s}.VIR3_clean.id_upper_oligo.uniq.1-80.bam.aligned_sequence_counts.txt 2> /dev/null | wc -l )
	echo -n ",${c}"
done
echo
