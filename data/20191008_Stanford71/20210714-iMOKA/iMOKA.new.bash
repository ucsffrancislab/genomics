...
Why did I un-scratch this script? Probably because the k-mer matrices are so huge.


date=$( date "+%Y%m%d%H%M%S%N" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --job-name="iMOKA" --output="${PWD}/iMOKA.${date}.out" --time=2880 --nodes=1 --ntasks=64 --mem=495G ${PWD}/iMOKA.new.bash --dir ${PWD}/16 --k 16


date=$( date "+%Y%m%d%H%M%S%N" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --job-name="iMOKA" --output="${PWD}/iMOKA.${date}.out" --time=2880 --nodes=1 --ntasks=64 --mem=495G ${PWD}/iMOKA.new.bash --dir ${PWD}/21 --k 21


date=$( date "+%Y%m%d%H%M%S%N" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --job-name="iMOKA" --output="${PWD}/iMOKA.${date}.out" --time=2880 --nodes=1 --ntasks=64 --mem=495G ${PWD}/iMOKA.new.bash --dir ${PWD}/31 --k 31



date=$( date "+%Y%m%d%H%M%S%N" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --job-name="iMOKA" --output="${PWD}/iMOKA.${date}.out" --time=2880 --nodes=1 --ntasks=64 --mem=495G ${PWD}/iMOKA.new.bash --dir ${PWD}/11 --k 11 --step reduce
