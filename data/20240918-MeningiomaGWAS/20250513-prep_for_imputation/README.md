...
#	20240918-MeningiomaGWAS/20250513-prep_for_imputation



I'm going to remove the ~2500 samples from the bed/bim/fam BEFORE imputation to see if it has any impact.
...
tail -n +2499 /francislab/data1/raw/20240918-MeningiomaGWAS/MENINGIOMA_GWAS_SHARED.fam | cut -d' ' -f1,2 > prep/select_samples
chmod -w prep/select_samples

sbatch --mail-user=$(tail -1 ~/.forward) --mail-type=FAIL --time 14-0 --nodes=1 --ntasks=4 --mem=30G --export=None --job-name=plink --wrap="module load plink; plink --bfile /francislab/data1/raw/20240918-MeningiomaGWAS/MENINGIOMA_GWAS_SHARED --keep prep/select_samples --make-bed --out prep/MENINGIOMA_GWAS_SHARED"
```

```
sed -i -e 's/MEN_1_G09_51388/MEN_1_G09_62050/g' -e 's/_/-/g' prep/MENINGIOMA_GWAS_SHARED.fam
chmod -w prep/MENINGIOMA_GWAS_SHARED.{bed,bim,fam}
```
...
```BASH
mkdir tmp ; cd tmp
scp c4:/francislab/data1/working/20240918-MeningiomaGWAS/20250513-prep_for_imputation/prep/*vcf.gz ./
```
...
https://imputation.biodatacatalyst.nhlbi.nih.gov/#!jobs/job-20250513-223652-537
...
Statistics:
Alternative allele frequency > 0.5 sites: 108,087
...
curl -sL https://imputation.biodatacatalyst.nhlbi.nih.gov/get/1625162/b3da5ebb74f882bc285aea2b35875ae6b2994f72477a24dba29bca430c997a4e | bash
curl -sL https://imputation.biodatacatalyst.nhlbi.nih.gov/get/1625158/5b2787b328212961e6fe0c15da2651b6fb0e3be376296b54c105fc9bb4a078f2 | bash
curl -sL https://imputation.biodatacatalyst.nhlbi.nih.gov/get/1625165/ca629050289eeefe9764fe522d21dcde518b6fb4aae8b8d6f5ba2c8dbbd47ee0 | bash
curl -sL https://imputation.biodatacatalyst.nhlbi.nih.gov/get/1625164/0631c874c111f9dda83c27d1e2ff2c77ec012b90fac1d4d5e302b9bf56b60e0d | bash
```
