...
dir=/francislab/data1/working/20230726-Illumina-CystEV/20230809-preprocess/out
...
echo -n "| hg38 unaligned Count |"
for s in ${samples} ; do
	c=$(cat ${dir}/${s}.format.umi.trim.Aligned.sortedByCoord.out.unmapped.fasta.gz.read_count.txt 2> /dev/null)
	echo -n " ${c} |"
done
echo
...
for s in ${samples} ; do
	c=$( cat out/${s}.RMHM.bam.q${q}.l${l}.aligned_sequence_counts.txt | datamash -W sum 1 2> /dev/null)
	echo -n " ${c} |"
done
echo
