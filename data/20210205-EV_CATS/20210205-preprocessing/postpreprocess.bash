...
for f in output/*.toTranscriptome.out.sorted.deduped.bam ; do
...
awk '(NR==FNR){t2g[$1]=$2}(NR!=FNR){print t2g[$1]}' ${transcript_gene} \
  output/*.toTranscriptome.out.sorted.deduped.bam.transcript_ids \
  | sort | uniq -c | sort -rn > gene_count
...
awk '(NR==FNR){t2g[$1]=$2}(NR!=FNR){print t2g[$1]}' ${transcript_gene} \
  output/SFHH0*.toTranscriptome.out.sorted.deduped.bam.transcript_ids \
  | sort | uniq -c | sort -rn > determined_gene_count.txt
...
~/.local/bin/featureCounts.bash -t miRNA_primary_transcript -g Name -a ${mirna_gff} \
  ...
  output/*.trimmed.STAR.Aligned.sortedByCoord.out.deduped.bam > post/STAR_mirna_miRNA_primary_transcript.tsv.log 2>&1
...
~/.local/bin/featureCounts.bash -t miRNA -g Name -a ${mirna_gff} \
  ...
  output/*.trimmed.STAR.Aligned.sortedByCoord.out.deduped.bam > post/STAR_mirna_miRNA.tsv.log 2>&1
...
#  output/*.trimmed.bowtie2.hg38.all.deduped.bam > post/bowtie2_all_mirna_miRNA_primary_transcript.tsv.log 2>&1
...
#  output/*.trimmed.bowtie2.hg38.all.deduped.bam > post/bowtie2_all_mirna_miRNA.tsv.log 2>&1
...
~/.local/bin/featureCounts.bash -t miRNA_primary_transcript -g Name -a ${mirna_gff} \
  ...
  output/*.trimmed.bowtie2.hg38.deduped.bam > post/bowtie2_mirna_miRNA_primary_transcript.tsv.log 2>&1
#  output/*.trimmed.bowtie2.hg38.bam > post/bowtie2_mirna_miRNA_primary_transcript.tsv.log 2>&1

~/.local/bin/featureCounts.bash -t miRNA -g Name -a ${mirna_gff} \
  -o post/bowtie2_mirna_miRNA.tsv \
  output/*.trimmed.bowtie2.hg38.deduped.bam > post/bowtie2_mirna_miRNA.tsv.log 2>&1
...
for f in output/*STAR.mirna.Aligned.sortedByCoord.out.deduped.bam output/*.bowtie{2,}.mirna{.all,}.deduped.bam ; do
