...
srun --chdir $PWD --cpus-per-task=8 \
	--begin=23:00 \
	--share --nice --exclude=n[0000-0029] \
	--job-name="pieces_merge-$base" \
	--output=$PWD/$now.pieces_merge.output  \
	--error=$PWD/$now.pieces_merge.error  \
		pieces_merge.sh *pieces.nobackup &
