...
parser.add_argument('-o', '--output', nargs=1, type=str, default='tumor_normal_pairs.tsv', help='output csv filename to %(prog)s (default: %(default)s)')
...
out=pd.DataFrame(columns=["Patient","Normal","Tumor"])
#print(out)

for patient in d.loc[d['tn']=="Normal"].patient.unique():
	normal=d.loc[(d['patient']==patient)&(d['tn']=="Normal")]
	tumors=d.loc[(d['patient']==patient)&(d['tn']=="Tumor")]
	if( len(tumors) > 0 ):
		#print( patient )
		#print( normal )
		#print( tumors )
		for index, tumor in tumors.iterrows():
			#print(normal.iloc[0].Z)
			#print(tumor.Z)
			out.loc[len(out.index)] = [patient,normal.iloc[0].Z,tumor.Z]
