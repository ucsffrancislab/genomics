...
if [ $( basename ${0} ) == "slurm_script" ] ; then
	...
	#	mkdir -p ${PWD}/logs
	#	date=$( date "+%Y%m%d%H%M%S%N" )
	#	sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-4%4 --job-name="MELT3" \
	#	--output="${PWD}/logs/MELT3.${date}-%A_%a.out" --time=10080 --nodes=1 --ntasks=4 --mem=40G ${PWD}/MELT_3_array_wrapper.bash
	...
	mkdir -p ${PWD}/logs
	date=$( date "+%Y%m%d%H%M%S%N" )
	sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-${max}%16 \
		--job-name="MELT3" --output="${PWD}/logs/MELT3.${date}-%A_%a.out" \
		--time=10080 --nodes=1 --ntasks=2 --mem=15G --gres=scratch:100G \
		$( realpath ${0} )

		#${PWD}/MELT_3_array_wrapper.bash
