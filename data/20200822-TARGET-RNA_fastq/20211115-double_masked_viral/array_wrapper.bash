...
OUT="/francislab/data1/working/20200822-TARGET-RNA_fastq/20211115-double_masked_viral/out"
mkdir -p ${OUT}
...
sample=$( sed -n "$line"p /francislab/data1/working/20200822-TARGET-RNA_fastq/20211115-double_masked_viral/sample_ids.txt )
echo $sample
...
date=$( date "+%Y%m%d%H%M%S" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-5%5 --job-name="DMV" --output="${PWD}/array.${date}-%A_%a.out" --time=480 --nodes=1 --ntasks=8 --mem=60G ${PWD}/array_wrapper.bash


date=$( date "+%Y%m%d%H%M%S" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-338%10 --job-name="DMV" --output="${PWD}/array.${date}-%A_%a.out" --time=480 --nodes=1 --ntasks=8 --mem=60G ${PWD}/array_wrapper.bash


date=$( date "+%Y%m%d%H%M%S" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --array=1-338%1 --job-name="DMV" --output="${PWD}/array.${date}-%A_%a.out" --time=480 --nodes=1 --ntasks=8 --mem=60G ${PWD}/array_wrapper.bash
