...
three=/francislab/data1/working/20230628-Costello/20240610-STAR_twopass_basic/out


samples=$( ls -1 ${three}/*.Log.final.out | xargs -I% basename % .Log.final.out )
...
echo -n "3 Uniquely mapped reads number"
for sample in ${samples} ; do
	value=$( grep "Uniquely mapped reads number" ${three}/${sample}.Log.final.out | awk -F\| '{print $2}' )
	echo -n ,${value}
done
echo
...
echo -n "3 Number of reads unmapped: too short"
for sample in ${samples} ; do
	value=$( grep "Number of reads unmapped: too short" ${three}/${sample}.Log.final.out | awk -F\| '{print $2}' )
	echo -n ,${value}
done
echo
...
echo -n "3 Number of reads unmapped: other"
for sample in ${samples} ; do
	value=$( grep "Number of reads unmapped: other" ${three}/${sample}.Log.final.out | awk -F\| '{print $2}' )
	echo -n ,${value}
done
echo
...
echo -n "3 Number of reads mapped to multiple loci"
for sample in ${samples} ; do
	value=$( grep "Number of reads mapped to multiple loci" ${three}/${sample}.Log.final.out | awk -F\| '{print $2}' )
	echo -n ,${value}
done
echo
...
echo -n "3 Number of reads mapped to too many loci"
for sample in ${samples} ; do
	value=$( grep "Number of reads mapped to too many loci" ${three}/${sample}.Log.final.out | awk -F\| '{print $2}' )
	echo -n ,${value}
done
echo
