...
Usage:
  Step 1: python phipseq_parallel.py prepare
  Step 2: sbatch array job (see generate_slurm_script())
  Step 3: python phipseq_parallel.py merge
"""
...
python phipseq_parallel.py process $SLURM_ARRAY_TASK_ID {num_batches}
"""
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python phipseq_parallel.py prepare [num_batches]")
        print("  python phipseq_parallel.py process <batch_id> <num_batches>")
        print("  python phipseq_parallel.py merge")
        sys.exit(1)
    ...
    if command == "prepare":
        ...
        log(f"  2. After jobs complete: python phipseq_parallel.py merge")
