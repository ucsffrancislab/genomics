...
parser$add_argument("-o", "--output", type="character", default=paste0(getwd(),"/PhIPseq_ChatGPT_2"),
...
---
title: "PhIP-seq (VirScan) QC, Normalization and Visualization — Fixed"
...
```{r}
print(dirname(opt$output))
print(paste0(dirname(opt$output),"/normalized_logCPM_fixed.csv"))
print(paste0(dirname(opt$output),"/sample_metadata_parsed_fixed.csv"))
write.csv(logCPM, file = paste0(dirname(opt$output),"/normalized_logCPM_fixed.csv"), quote = FALSE)
write.csv(sample_metadata, file = paste0(dirname(opt$output),"/sample_metadata_parsed_fixed.csv"), row.names = FALSE)
