...
rawdir=/francislab/data1/raw/20241224-Illumina-PhIP/fastq
cutdir=/francislab/data1/working/20241224-Illumina-PhIP/20241224a-cutadapt/out
bowdir=/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out




#manifest=/francislab/data1/raw/20241224-Illumina-PhIP/manifest.csv
#sample,desc,fastq
...
#manifest=/francislab/data1/working/20241224-Illumina-PhIP/20241224c-PhIP/manifest.all.csv
#subject,sample,bampath,type,study,group,age,sex
#14061-01,14061-01dup,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S124.VIR3_clean.1-84.bam,glioma serum,,case,,
#14061-01,14061-01,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S116.VIR3_clean.1-84.bam,glioma serum,,case,,
#14091-01,14091-01dup,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S154.VIR3_clean.1-84.bam,glioma serum,,case,,
#14091-01,14091-01,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S146.VIR3_clean.1-84.bam,glioma serum,,case,,

#manifest=/francislab/data1/raw/20241224-Illumina-PhIP/L1_full_covariates_Vir3_phip-seq_GBM_p1_MENPEN_p13_12-4-24hmh.csv


#join -j 2 --header -t, <( cat /francislab/data1/working/20241224-Illumina-PhIP/20241224c-PhIP/manifest.all.csv | ( head -1 && tail -n +2 | sort -t, -k2,2 )) <( cat /francislab/data1/raw/20241224-Illumina-PhIP/manifest.csv | ( head -1 && tail -n +2 | sort -t, -k2,2 ) ) > /francislab/data1/raw/20241224-Illumina-PhIP/manifest.TEST.csv - problematic

#join -j 2 -t, <( tail -n +2 /francislab/data1/working/20241224-Illumina-PhIP/20241224c-PhIP/manifest.all.csv | sort -t, -k2,2 ) <( tail -n +2 /francislab/data1/raw/20241224-Illumina-PhIP/manifest.csv | sort -t, -k2,2 ) > /francislab/data1/raw/20241224-Illumina-PhIP/manifest.TEST.csv
#sed -i '1isample,subject,bampath,type,study,group,age,sex,sample,fastq' /francislab/data1/raw/20241224-Illumina-PhIP/manifest.TEST.csv


#manifest=/francislab/data1/raw/20241224-Illumina-PhIP/manifest.TEST.csv
#sample,subject,bampath,type,study,group,age,sex,sample,fastq
#074KBP,074KBP,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S66.VIR3_clean.1-84.bam,pemphigus serum,,Non Endemic Control,,,S66,074KBP_121724_S66_L001_R1_001.fastq.gz
#074KBPdup,074KBP,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S74.VIR3_clean.1-84.bam,pemphigus serum,,Non Endemic Control,,,S74,074KBPdup_121724_S74_L001_R1_001.fastq.gz
#1322,1322,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S67.VIR3_clean.1-84.bam,meningioma serum,,,,,S67,1322_121724_S67_L001_R1_001.fastq.gz
#1322dup,1322,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S75.VIR3_clean.1-84.bam,meningioma serum,,,,,S75,1322dup_121724_S75_L001_R1_001.fastq.gz
#1323,1323,/francislab/data1/working/20241224-Illumina-PhIP/20241224b-bowtie2/out/S70.VIR3_clean.1-84.bam,meningioma serum,,,,,S70,1323_121724_S70_L001_R1_001.fastq.gz
...
for s in ${samples} ; do
	#s=${s#S}
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $5}' ${manifest} )

	#v=$( awk -F"," -v sample=${s} 'BEGIN{FPAT="([^,]*)|(\"[^\"]+\")"}( $9 == sample ){print $2}' ${manifest} )
	echo -n ",${v}"
done
echo
...
for s in ${samples} ; do
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	#s=${s#S}
	#v=$( awk -F"," -v sample=${s} 'BEGIN{FPAT="([^,]*)|(\"[^\"]+\")"}( $9 == sample ){print $4}' ${manifest} )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $4}' ${manifest} )
	echo -n ",${v}"
done
echo
...
for s in ${samples} ; do
#	s=${s#S}
#	v=$( awk -F"," -v sample=${s} 'BEGIN{FPAT="([^,]*)|(\"[^\"]+\")"}( $9 == sample ){print $5}' ${manifest} )
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $14}' ${manifest} )
	echo -n ",${v}"
done
echo
...
for s in ${samples} ; do
#	s=${s#S}
#	v=$( awk -F"," -v sample=${s} 'BEGIN{FPAT="([^,]*)|(\"[^\"]+\")"}( $9 == sample ){print $6}' ${manifest} )
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $15}' ${manifest} )
	echo -n ",${v}"
done
echo
...
for s in ${samples} ; do
#	s=${s#S}
#	v=$( awk -F"," -v sample=${s} 'BEGIN{FPAT="([^,]*)|(\"[^\"]+\")"}( $9 == sample ){print $7}' ${manifest} )

	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $16}' ${manifest} )
	echo -n ",${v}"
done
echo
...
#	v=$[ ( ${s} - 1 ) / 96 ]
	...
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){print $1}' ${manifest} )
	echo -n ",${v}"
done
echo
...
#	v=$[ ( ${s} - 1 ) % 8 + 1 ]
	...
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){x=substr($2,1,1); print x}' ${manifest} | tr 'A-H' '1-8' )
	echo -n ",${v}"
done
echo
...
#	v=$[ ( ${s} - 1 ) / 8 % 12 + 1 ]
	...
	s=$( echo ${s} | awk '{sub("S","");printf("IDX%03d",$0)}' )
	v=$( awk -F, -v sample=${s} '( $7 == sample ){x=substr($2,2,2); print int(x)}' ${manifest} )
	echo -n ",${v}"
done
echo
