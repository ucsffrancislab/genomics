...
echo -n "| Pear Assembled Read Count |"
for s in ${samples} ; do
	c=$(cat pear/${s}.quality.assembled.fastq.gz.read_count.txt 2> /dev/null)
	echo -n " ${c} |"
done
echo
...
echo -n "| Pear Assembled % Read Count |"
for s in ${samples} ; do
	n=$(cat pear/${s}.quality.assembled.fastq.gz.read_count.txt 2> /dev/null)
...
echo -n "| Pear Unassembled Read Count |"
for s in ${samples} ; do
	c=$(cat pear/${s}.quality.unassembled.forward.fastq.gz.read_count.txt 2> /dev/null)
	echo -n " ${c} |"
done
echo
...
echo -n "| Pear Unassembled % Read Count |"
for s in ${samples} ; do
	n=$(cat pear/${s}.quality.unassembled.forward.fastq.gz.read_count.txt 2> /dev/null)
	d=$(cat ${dir}/${s}.quality.R1.fastq.gz.read_count.txt 2> /dev/null)
	c=$( echo "scale=2; 100 * ${n} / ${d}" | bc -l 2> /dev/null)
	echo -n " ${c} |"
done
