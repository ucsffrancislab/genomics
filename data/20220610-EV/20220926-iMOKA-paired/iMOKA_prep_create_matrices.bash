...
#		cat ${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | grep -f sample_selection/${s} | sed -E 's/Primary|Recurrent/tumor/' | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' > TumorControl/${k}/${s}/create_matrix.tsv
		...
		mkdir -p PrimaryRecurrent/${k}
		ln -s ../../${k}/preprocess PrimaryRecurrent/${k}/preprocess
		cat ${k}/create_matrix.tsv | \grep -E "Primary|Recurrent" | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > PrimaryRecurrent/${k}/create_matrix.tsv

		mkdir -p PrimaryRecurrentControl/${k}
		ln -s ../../${k}/preprocess PrimaryRecurrentControl/${k}/preprocess
		cat ${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > PrimaryRecurrentControl/${k}/create_matrix.tsv

		mkdir -p TumorControl/${k}
		ln -s ../../${k}/preprocess TumorControl/${k}/preprocess
		cat ${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | sed -E 's/Primary|Recurrent/tumor/' | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' > TumorControl/${k}/create_matrix.tsv
