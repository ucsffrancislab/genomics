...
#	20220610-EV/20220914-iMOKA

Quality 15 filtered, paired-end alignment


Link raw files as fasta.gz
```
DIR=/francislab/data1/working/20220610-EV/20220913-preprocess/out
RAW=${PWD}/raw
mkdir -p $RAW
for f in ${DIR}/*.quality15.format.umi.t1.t2.t3.notphiX.readname.hg38.rx.name.mated.marked.deduped.fa.gz ; do
l=$( basename $f .quality15.format.umi.t1.t2.t3.notphiX.readname.hg38.rx.name.mated.marked.deduped.fa.gz )
...
date=$( date "+%Y%m%d%H%M%S%N" )
sbatch --mail-user=$(tail -1 ~/.forward)  --mail-type=FAIL --job-name="iMOKA" --output="${PWD}/logs/iMOKA.dump.${date}.out" --time=720 --nodes=1 --ntasks=32 --mem=240G ${PWD}/iMOKA_dump.bash --k 16
