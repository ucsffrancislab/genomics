...
#	Previous 20230726 way ....
#cat ${PWD}/{out,blank}/${k}/create_matrix.tsv | grep -vs SFHH00 > ${PWD}/V01/${k}/create_matrix.tsv


#	Create a create matrix of the selected training ids with the appropriate label

##join -1 2 -2 1 <( sort -k2,2 ${PWD}/out/${k}/create_matrix.tsv)  <( sort -k1,1 ${PWD}/train_ids.tsv )| awk 'BEGIN{OFS="\t"}{print $2,$1,$4}' > ${PWD}/V01/${k}/create_matrix.tsv
##join -1 2 -2 1 <( sort -k2,2 ${PWD}/out/${k}/create_matrix.tsv)  <( sort -k1,1 ${PWD}/blank_ids.tsv )| awk 'BEGIN{OFS="\t"}{print $2,$1,$4}' >> ${PWD}/V01/${k}/create_matrix.tsv
...
#	prep to dump
#singularity exec ${img} iMOKA_core create -i ${PWD}/V01/${k}/create_matrix.tsv -o ${PWD}/V01/${k}/create_matrix.json

ln -s ${PWD}/V01/${k}/matrix.json ${PWD}/V01/${k}/create_matrix.json

#	dump rescaled kmers to be used in the zscore algorithm
singularity exec ${img} iMOKA_core dump -i ${PWD}/V01/${k}/create_matrix.json -o ${PWD}/V01/${k}/kmers.rescaled.tsv

gzip ${PWD}/V01/${k}/kmers.rescaled.tsv

#	dump raw kmers to be put back based on zscore results
singularity exec ${img} iMOKA_core dump --raw -i ${PWD}/V01/${k}/create_matrix.json -o ${PWD}/V01/${k}/kmers.raw.tsv
...
#	convert tsv to csv plus add header and merge the last 6 as blanks as "input"
zcat ${PWD}/V01/${k}/kmers.raw.tsv.gz | awk 'BEGIN{FS="\t";OFS=","}(NR==1){s="id"; for(i=2;i<=(NF-6);i++){s=s","$i};print s,"input"} (NR>2){s=$1; for(i=2;i<=(NF-6);i++){s=s","$i};print s,($(NF-5)+$(NF-4)+$(NF-3)+$(NF-2)+$(NF-1)+$NF)}' > ${PWD}/V01/${k}/kmers.raw.count.csv

#	convert tsv to csv plus add header and merge the last 6 as blanks as "input"
zcat ${PWD}/V01/${k}/kmers.rescaled.tsv.gz | awk 'BEGIN{FS="\t";OFS=","}(NR==1){s="id"; for(i=2;i<=(NF-6);i++){s=s","$i};print s,"input"} (NR>2){s=$1; for(i=2;i<=(NF-6);i++){s=s","$i};print s,($(NF-5)+$(NF-4)+$(NF-3)+$(NF-2)+$(NF-1)+$NF)}' > ${PWD}/V01/${k}/kmers.rescaled.count.csv
...
elledge_Zscore_analysis.R ${PWD}/V01/${k}/kmers.rescaled.count.csv



#	rename headers by adding _z to zscore results
sed -i '1 s/,/_z,/g' ${PWD}/V01/${k}/kmers.rescaled.count.Zscores.csv

#	reorder columns
awk 'BEGIN{FS=OFS=","}{print $(NF-2),$0}' ${PWD}/V01/${k}/kmers.rescaled.count.Zscores.csv > ${PWD}/V01/${k}/kmers.rescaled.count.Zscores.reordered.csv

gzip ${PWD}/V01/${k}/kmers.rescaled.count.Zscores.csv
...
gzip ${PWD}/V01/${k}/kmers.raw.count.csv
gzip ${PWD}/V01/${k}/kmers.rescaled.count.csv
gzip ${PWD}/V01/${k}/kmers.rescaled.count.Zscores.reordered.csv

#	loop through giant matrix. Split and keep only those with all absolute zscores larger than 3.5
awk -v dir="${PWD}/zscores_filtered_V01/${k}/preprocess" 'BEGIN{FS=",";OFS="\t"}
(NR==1){ 
split($0,columns,",")
...
} } }' ${PWD}/V01/${k}/kmers.raw.count.Zscores.reordered.joined.csv
