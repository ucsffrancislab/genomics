...
join -1 2 -2 1 <( sort -k2,2 ${PWD}/out/${k}/create_matrix.tsv)  <( sort -k1,1 ${PWD}/train_ids.tsv )| awk 'BEGIN{OFS="\t"}{print $2,$1,$4}' > ${PWD}/dump/${k}/create_matrix.tsv
join -1 2 -2 1 <( sort -k2,2 ${PWD}/out/${k}/create_matrix.tsv)  <( sort -k1,1 ${PWD}/blank_ids.tsv )| awk 'BEGIN{OFS="\t"}{print $2,$1,$4}' >> ${PWD}/dump/${k}/create_matrix.tsv
...
zcat ${PWD}/dump/${k}/kmers.raw.tsv.gz | awk 'BEGIN{FS="\t";OFS=","}(NR==1){s="id"; for(i=2;i<=(NF-6);i++){s=s","$i};print s,"input"} (NR>2){s=$1; for(i=2;i<=(NF-6);i++){s=s","$i};print s,($(NF-5)+$(NF-4)+$(NF-3)+$(NF-2)+$(NF-1)+$NF)}' > ${PWD}/dump/${k}/kmers.raw.count.csv

#	convert tsv to csv plus add header and merge the last 6 as blanks as "input"
zcat ${PWD}/dump/${k}/kmers.rescaled.tsv.gz | awk 'BEGIN{FS="\t";OFS=","}(NR==1){s="id"; for(i=2;i<=(NF-6);i++){s=s","$i};print s,"input"} (NR>2){s=$1; for(i=2;i<=(NF-6);i++){s=s","$i};print s,($(NF-5)+$(NF-4)+$(NF-3)+$(NF-2)+$(NF-1)+$NF)}' > ${PWD}/dump/${k}/kmers.rescaled.count.csv
