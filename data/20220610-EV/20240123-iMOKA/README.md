...
#	20220610-EV/20240123-iMOKA

Analyze both Glioma datasets together 
...
paired end
/francislab/data1/working/20220610-EV/20230814-preprocess/out/SFHH011*.format.umi.trim.Aligned.sortedByCoord.out.umi_tag.fixmate.deduped.R1.fastq.gz
```

```
mkdir -p in
for f in /francislab/data1/working/20210428-EV/20230605-preprocessing/out/SFHH005*.umi_tag.dups.deduped.fastq.gz \
  /francislab/data1/working/20220610-EV/20230814-preprocess/out/SFHH011*.format.umi.trim.Aligned.sortedByCoord.out.umi_tag.fixmate.deduped.*.fastq.gz ; do
echo $f
l=$(basename $f)
...
```
\rm source.all.tsv
awk -v pwd=${PWD} 'BEGIN{FS=",";OFS="\t"}{ if(system("test -f in/"$1".fastq.gz")==0){ print $1,$2,pwd"/in/"$1".fastq.gz" } }' 20210428.metadata.csv >> source.all.tsv
...
cat /francislab/data1/working/20230726-Illumina-CystEV/20230815-iMOKA/source.blanks.tsv >> source.all.tsv

```



NOTE these are 2-60 MB
```
/francislab/data1/working/20210428-EV/20230605-preprocessing/out/SFHH005*.deduped.bam
```

and these are much bigger 200-1300MB

```
/francislab/data1/working/20220610-EV/20230814-preprocess/out/SFHH011*deduped.bam
```
