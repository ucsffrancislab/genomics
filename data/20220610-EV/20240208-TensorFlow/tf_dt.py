...
train_ds_pd=dataset.loc[train_ids].reset_index().set_index("level_0")
test_ds_pd=dataset.loc[test_ids].reset_index().set_index("level_0")
...
#ValueError: `fit` cannot consume Pandas' dataframes directly. Instead, use the `pd_dataframe_to_tf_dataset` utility function. For example: `model.fit(


#	Can I just read into a tf.data.Dataset?
