...
#		cat ${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | grep -f sample_selection/${s} | sed -E 's/Primary|Recurrent/tumor/' | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' > TumorControl/${k}/${s}/create_matrix.tsv
		...
		mkdir -p out/PrimaryRecurrent/${k}
		ln -s ../../${k}/preprocess out/PrimaryRecurrent/${k}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent" | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > out/PrimaryRecurrent/${k}/create_matrix.tsv

		mkdir -p out/PrimaryRecurrentControl/${k}
		ln -s ../../${k}/preprocess out/PrimaryRecurrentControl/${k}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > out/PrimaryRecurrentControl/${k}/create_matrix.tsv

		mkdir -p out/TumorControl/${k}
		ln -s ../../${k}/preprocess out/TumorControl/${k}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | sed -E 's/Primary|Recurrent/tumor/' | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' > out/TumorControl/${k}/create_matrix.tsv
