...
if [ ! -f sample_selection/Primary ] ; then
	cat out/11/create_matrix.tsv | awk -F"\t" '( $3 == "Primary" ){print $2".tsv"}' > sample_selection/Primary
...
if [ ! -f sample_selection/Recurrent ] ; then
	cat out/11/create_matrix.tsv | awk -F"\t" '( $3 == "Recurrent" ){print $2".tsv"}' > sample_selection/Recurrent
...
if [ ! -f sample_selection/control ] ; then
	cat out/11/create_matrix.tsv | awk -F"\t" '( $3 == "control" ){print $2".tsv"}' > sample_selection/Control
...
for k in 11 16 21 31 ; do
	...
	for s in 14a 14b 14c ; do
		...
		mkdir -p out/PrimaryRecurrent/${k}/${s}
		ln -s ../../../${k}/preprocess out/PrimaryRecurrent/${k}/${s}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent" | grep -f sample_selection/${s} | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > out/PrimaryRecurrent/${k}/${s}/create_matrix.tsv

		mkdir -p out/PrimaryRecurrentControl/${k}/${s}
		ln -s ../../../${k}/preprocess out/PrimaryRecurrentControl/${k}/${s}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | grep -f sample_selection/${s} | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' | sort | uniq > out/PrimaryRecurrentControl/${k}/${s}/create_matrix.tsv

		mkdir -p out/TumorControl/${k}/${s}
		ln -s ../../../${k}/preprocess out/TumorControl/${k}/${s}/preprocess
		cat out/${k}/create_matrix.tsv | \grep -E "Primary|Recurrent|control" | grep -f sample_selection/${s} | sed -E 's/Primary|Recurrent/tumor/' | awk -F"\t" '(system("test -f " $1".sorted.bin")==0)' > out/TumorControl/${k}/${s}/create_matrix.tsv
