...
#	change ".fasta" to ".mask.fasta"
		n=''
		if [ -f out/split.${split}/${a}.split.${size}.fasta.base_count.txt ] ; then
			n=$( awk '($2=="N"){print $1}' out/split.${split}/${a}.split.${size}.fasta.base_count.txt )
		fi
		if [ -z "${n}" ] ; then
			n=0
		fi
...
#	change ".fasta" to ".mask.fasta"
		n=''
		if [ -f out/split.${split}/${a}.masked.split.${size}.fasta.base_count.txt ] ; then
			n=$( awk '($2=="N"){print $1}' out/split.${split}/${a}.masked.split.${size}.fasta.base_count.txt )
		fi
		if [ -z "${n}" ] ; then
			n=0
		fi
...
#	change ".fasta" to ".mask.fasta"
		...
		if [ -f out/split.${split}/${a}.masked.split.${size}.fasta.base_count.txt ] && [ -f out/masks/${a}.masked.fasta.base_count.txt ] ; then
			mmn=$( awk '($2=="N"){print $1}' out/split.${split}/${a}.masked.split.${size}.fasta.base_count.txt )
