...
for f in /francislab/data1/refs/refseq/complete-20210920/complete_genomes/*fa; do

	#echo $f
	
	#accession=$( head -1 $f | sed -e 's/_/ /g' -e 's/^>NC />NC_/' -e 's/^>AC />AC_/' -e 's/^>//' | awk '{print $1}' )
	accession=$( head -1 $f | sed -e 's/_/ /g' -e 's/ /_/' -e 's/^>//' | awk '{print $1}' )
	echo $accession
	...
	if [ ! -f ${l} ] ; then
		...
		cat ${f} | sed -e '1s/_/ /g' -e '1s/ /_/' > ${l}
		chmod +w ${l}
	fi

	m=masks/$( basename $l )
	if [ ! -f ${m}.cat ] ; then
		echo ~/.local/RepeatMasker/RepeatMasker -pa 8 -dir ${PWD}/masks ${PWD}/$l
